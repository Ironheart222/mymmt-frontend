...
export const MappedCoreVideo = () => {
	...
	const [openLessonMapModel, setOpenLessonMapModel] = React.useState<boolean>(false);
	const [openEditMapModel, setOpenEditMapModel] = React.useState<boolean>(false);
	...
	React.useEffect(() => {
		...
		dispatch(getMappedLesson({_request:30,adminClient}));
	}, []);
	...
	const tableColumns: MUIDataTableColumn[] = [
		...
		},
		{
			...
			options: {
				setCellProps: () => ({ style: { minWidth: "180px" } }),
				customBodyRender: (value) => {
					return value?.category_name ? value?.category_name : "-"
				}
			}
		},
		{
		...
		mappedLessonData && mappedLessonData.map((val: any, index: number) => {
			data.push(
				[
					...
					val.vdo_cipher_video ? val.vdo_cipher_video : null,
					val.worksheet_detail ? val.worksheet_detail : null,
					val.category_detail ? val.category_detail : null,
